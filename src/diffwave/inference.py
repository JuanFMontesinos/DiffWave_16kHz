...
def main(args):
    ...
    if args.compare:
        ...
        gt_wav_path = args.spectrogram_path.replace('.npy', '').replace('.spec', '')
        gt_audio, _ = librosa.load(gt_wav_path, sr=base_params.sample_rate)
        ld.waveplot(gt_audio, sr=base_params.sample_rate)
        ld.waveplot(audio[0].cpu().numpy(), sr=base_params.sample_rate)
        plt.show()
